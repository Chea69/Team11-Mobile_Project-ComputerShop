...
NexusBuilderPart? _chosen(BuilderStep step, BuilderState sel) => switch (step) {
  ...
  void _assign(NexusBuilderPart part) {
    ...
    if (index < _labels.length - 1) {
      ...
      Future<void>.delayed(const Duration(milliseconds: 220)).then((_) {
        if (mounted) {
          setState(() => index = (stepFrom + 1).clamp(0, _labels.length - 1));
        }
      });
    }
    setState(() {});
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> review() async {
      if (issues.isNotEmpty) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(issues.first.toUpperCase())));
        return;
      }
      if (totalSpend() == 0) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('SELECT PARTS FIRST')));
        return;
      }
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: parts.isEmpty
              ...
              : ListView.builder(
                  ...
                  padding: const EdgeInsets.fromLTRB(14, 12, 14, 200),
                  itemCount: parts.length,
                  itemBuilder: (_, i) {
                    final part = parts[i];
                    final mark = (_chosen(step, sel)?.id == part.id);

                    Color borderTone() => mark
                        ? NexusPalette.cyan
                          ...
                          children: [
                            ClipRRect(
                              ...
                              child: SizedBox(
                                width: 64,
                                height: 64,
                                child: NexusNetworkImage(imageUrl: part.image),
                              ),
                            ),
        ...
        DecoratedBox(
          decoration: BoxDecoration(
            color: NexusPalette.darkSurface.withValues(alpha: .93),
            border: Border(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color: issues.isEmpty
                                  ...
                                  : Colors.redAccent.withValues(alpha: .52),
                            ),
                          ),
                          child: Row(
                        ...
                        child: index == _labels.length - 1
                            ? FilledButton(
                                style: FilledButton.styleFrom(
                                  ...
                                  backgroundColor: NexusPalette.magenta
                                      .withValues(alpha: .85),
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(14),
                                  ),
...
class _BuildSummarySheetState extends State<BuildSummarySheet> {
  ...
  Widget build(BuildContext context) {
        ...
        child: Column(
          ...
          children: [
            ...
            TextButton(
              ...
              child: Text('GO TO BUILDER', style: GoogleFonts.jetBrainsMono()),
            ),
          ],
        ),
      );
    ...
    final divider = Theme.of(context).dividerColor.withValues(alpha: .45);
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: Colors.lightGreenAccent.withValues(alpha: .45),
                      ),
                    ),
                    child: Row(
                      children: [
